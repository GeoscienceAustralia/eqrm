...
def stats(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # print "STATS %s(%s) Running time=%s" % (func.__name__, str(*args),
        # str(tdiff))
        ...
        if os.path.exists(log.log_filename):
            ...
            # print "STATS %s(%s) SVN version=%s date=%s modified=%s" %
            # (func.__name__, str(*args), version, date, modified)
            ...
            # print "STATS %s(%s) Output=%s" % (func.__name__, str(*args),
            # memory_usage_locations)

            # Append to CSV file (use the current logfile output directory)
            # Note: analysis.main changes the value of log.log_filename per
            # setdata file

...
def modified_Wells_and_Coppersmith_94_rup_width(dip, Mw, area, max_rup_width,
    ...
    f = where(
        Mw > 5.5,
        sqrt(sqrt(1 + 2 * (Mw_mod - 5.5) * sin(dip * pi / 180.))) ** -1,
        1.0)
...
def Strasser_et_al_2010_interface_rup_width(
        dip, Mw, area, max_rup_width, **kwargs):
    """
...
def Strasser_et_al_2010_intraslab_rup_width(
        dip, Mw, area, max_rup_width, **kwargs):
    """
    From Scaling of the Source Dimensions of Interface and Intraslab
    Subduction-zone Earthquakes with Moment Magnitude.

...
attribute_conversions = copy.deepcopy(attribute_conversions)
attribute_conversions['CD_CODE'] = int
...
class Building_db_writer(object):
    ...
    def write_row(self, aggregate, row_aggrigate, survey_factor_sum, UFI):
        ...
        row.append(
            aggregate[self.aggregate_on_index['STRUCTURE_CLASSIFICATION']])
        ...
        row.append(
            aggregate[self.aggregate_on_index['HAZUS_STRUCTURE_CLASSIFICATION']])
        row.append(
            row_aggrigate[self.average_on_index['CONTENTS_COST_DENSITY']])
        row.append(
            row_aggrigate[self.average_on_index['BUILDING_COST_DENSITY']])
        ...
        row.append(aggregate[self.aggregate_on_index['CD_CODE']])
        row.append(UFI)

...
within the main driving function \typefunc{do}{\_ana}{lysis} which
...
matrix is \typevar{S}{}{A} or \typevar{SA}{ro}{ck} in the code)
...
event catalogue (see \sref{sec:event-catalogue}) and a calculation
of distance (see \sref{sec:attn-dist}). The $T_o$ are determined
by the user-defined vector parameter \typepar{per}{io}{ds} in
\typeself{set}{da}{ta}. Computationally the above calculation of
...
\typepar{per}{io}{ds}. \item The computation of the
$S_a(T_o,r_m,R)$ at each site (\typefunc{do}{\_atten}{uation})
...
catalogue is computed by \typefunc{do}{\_all}{dist}. Recall that
in the description of an event (see \tref{tab:evntdb-columns})
...
\typeself{E}{QR}{M} application currently uses the first (i.e.
...
\sref{attn:uncertainty} and the use of multiple attenuation models
is described in \sref{sec:attn-multi-attnmodels}.
...
by using the \typeself{set}{da}{ta} parameter
\typepar{attn}{\_reg}{ion} (see \tref{tab:attn-flags}).
...
the file \typeparfile{attn\_toro}{\_midcontinent}{\_momag}{.txt}
(for the mid-continent region). The preparation of coefficients is
undertaken by \typefunc{attn}{\_prepcoeff}{\_toro} (called by
\typefunc{prep}{\_at}{tn}) and the attenuation computed by
\typefunc{attn}{\_allper}{vec\_toro} (called by
\typefunc{do}{\_atten}{uation}).
...
\typefunc{attn}{\_preptoro}{\_aleatory1}) and a path dependant
component $\sigma_{a,\Rjb}$ (see \citet[Table 4]{dr_Toro97a} or
\typefunc{attn}{\_preptoro}{\_aleatory2}) as follows:
\begin{equation}
...
region is controlled by the flag \typepar{attn}{\_reg}{ion} whose
functionality is described by \sref{sec:attn-multi-attnmodels} and
\tref{tab:attn-regions}. The attenuation formulae is:
\begin{equation}
...
the file \typefunc{attn}{\_allpervec}{\_gaull} (for the $PGA$
case) and \typefunc{attn}{\_mmi}{\_gaull} (for the $\Imm$ case).
...
\typefunc{attn}{\_allpervec}{\_gaull} and the `extension' to a
complete $S_a$ is undertaken by \typefunc{aus}{\_pesh}{new}.
...
in the file \typeparfile{attn}{\_atkboore}{\_momag}{.txt}. The
preparation of coefficients is undertaken by
\typefunc{attn}{\_prepcoeff}{\_atkboore} and the attenuation
computed by \typefunc{attn}{\_allpervec}{\_atkboore}.
...
\typefunc{attn\_prep}{\_atkboore}{\_aleatory}).
...
the files
\typeparfile{attn\_sadigh}{\_coeff\_momag}{\_less65}{.txt} and
\typeparfile{attn\_sadigh}{\_coeff}{\_momag\_great65}{.txt} for
\mbox{$r_m \leq 6.5$} and \mbox{$r_m > 6.5$} respectively. The
preparation of coefficients is undertaken by
\typefunc{attn}{\_prepcoeff}{\_sadigh} and the attenuation
computed by \typefunc{attn}{\_allpervec}{\_sadigh}.
...
\typeparfile{attn\_sadigh}{\_coeff\_momag}{\_less65}{.txt} and
\typeparfile{attn\_sadigh}{\_coeff}{\_momag\_great65}{.txt}. The
preparation of the coefficients is conducted by
\typefunc{attn\_prep}{\_sadigh}{\_uncertainty}. For the purpose of
...
modified by changing \typepar{attn}{\_reg}{ion} (see
\sref{sec:attn-multi-attnmodels} and \tref{tab:attn-regions}).
...
and in the file \typeparfile{attn\_somer}{\_nonrift}{\_hori}{.txt}
(for the Nonrift -- Horizontal case). The preparation of
coefficients is undertaken by
\typefunc{attn}{\_prepcoeff}{\_somerville} (called by
\typefunc{prep}{\_at}{tn}) and the attenuation computed by
\typefunc{attn}{\_allpervec}{\_somerville} (called by
\typefunc{do}{\_atten}{uation}).
...
\typepar{var}{\_attn}{\_flag}. The aleatory uncertainty is based
on estimations of $\sigma_{log(S_a)}$ (see
\sreftwo{attn:atten-formula}{attn:implementation}) and is
incorporated within \typefunc{do}{\_atten}{uation}.
...
control the uncertainty method is \typepar{var}{\_attn}{\_method}
and \tref{tab:attn-varmethods} illustrates all of the aleatory
...
\subsection{Sampling the probability density function of the response spectral acceleration\index{response spectral acceleration} (spawning)}

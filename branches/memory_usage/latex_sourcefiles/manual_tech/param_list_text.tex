...
DEFAULT = \texttt{\$EQRMDATADIR/data} \\
Directory used to save to and load from event set data files.\\
%---------------------------------------------------------
\hline \vspace{0.1em} \texttt{event\_data\_handler}: \\
...
on. \\
 ...
 save to \texttt{data\_dir}, and exit \\
 \hspace{0.5em} \texttt{`load'} $\Rightarrow$ load event set from
 \texttt{data\_dir} (previously generated using \texttt{`save'}) \\

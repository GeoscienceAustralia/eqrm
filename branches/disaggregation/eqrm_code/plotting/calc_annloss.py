...
def calc_annfatalities(fatalities_matrix, event_activity):
    """Computes the annualised fatalities from a standard probabilistic EQRM risk run.

    INPUTS:
    fatalities_matrix    Contains the loss estimates in dollars for each building
                   multiplied by the survey factor. Note that the matrix has one
                   row for each event and one column for each building (MxN).
    event_activity the activity of each of the simulated events (Mx1).

    Returns a tuple (ann_fatalities, cum_ann_fatalities) where:
    ann_fatalities       is the annualised fatalities in numbers
    cum_ann_fatalities   is a tuple (return, fnumbers) where:
                       return  is the return period
                       fnumbers is the cumulative annual fatalities in numbers
    ...
    # reorder the loss values and activity
    # and get the cumulative activity values
    fatalities_by_actitivy = num.sum(fatalities_matrix, axis=1)
    (trghzd_agg, sort_loss_activity,
     cum_event_activity) = acquire_riskval(fatalities_by_actitivy,
                                              event_activity, 0)
    #print "sort_loss_activity", sort_loss_activity
    #print "cum_event_activity", cum_event_activity
    ...
    tmp_ann_fatalities = integrate_backwards(sort_loss_activity, prob_exceed)

    ann_fatalities = num.array([tmp_ann_fatalities[0]])

    return_period = 1.0/ cum_event_activity
    #assert scipy.allclose(retrn_per1, retrn_period)
    cum_ann_fatalities = num.array([return_period, tmp_ann_fatalities])

    return (ann_fatalities, cum_ann_fatalities)

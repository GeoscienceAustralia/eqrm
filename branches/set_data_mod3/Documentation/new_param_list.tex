...
EQRM for several parameters. These are also indicated below when
applicable.
 There is an example
...
\begin{supertabular}{lp{0.6\textwidth}}
\typepar{prob\_azimuth\_in\_zones}{}{} &     Predominant azimuth of events. Note that this can be
a single value (used in all source zones) or a vector with different elements for each source zone.\\
\typepar{prob\_delta\_azimuth\_in\_zones}{}{}   &   Azimuth range for events (i.e. \texttt{azimuth}
...
\typepar{prob\_number\_of\_events\_in\_zones}{}{}  &  List whose elements represent the
desired number of simulated evens for each generation zone.\\
\typepar{prob\_dip\_in\_zones}{}{} &    Dip of virtual faults.  Note that this can be a single
...
the magnitude recurence probability density function. \\
 \end{supertabular}
...
(or GMPEs) to be used. Current choices are; \\
...
\typepar{atten\_model\_weights}{}{}    &  A list of the weights to
be applied to each of the GMPEs in
\texttt{atten\_models}.  Note, $\sum  atten\_model\_weights=1$. \\
\typepar{atten\_aggregate\_Sa\_of\_atten\_models}{}{}   & Set to
...
\typepar{atten\_override\_RSA\_shape}{}{}  &   Use GMPE for PGA only and change shape of RSA as follows\\
 & \hspace{0.5em} \texttt{'Aust\_standard\_Sa'} $\Rightarrow$
use RSA shape from Australian earthquake loading standard; \\
 & \hspace{0.5em} \texttt{'HAZUS\_Sa'}  $\Rightarrow$ use shape defined by HAZUS;\\
...
\texttt{True} to  smooth $S_a$: \\
 \typepar{atten\_log\_sigma\_eq\_weight}{}{} &  Used to separate the
 total GMPE aleatory uncertainty or $\sigma_{total}$  into intra-
 $\sigma_{intra}$  and inter-  $\sigma_{inter}$ event components.
 More specifically, \typepar{atten\_log\_sigma\_eq\_weight}{}{}
 represents the fraction of $\sigma_{total}$ associated with
 intra-event aleatory uncertainty. Similarly, 1 -
 \typepar{atten\_log\_sigma\_eq\_weight}{}{} is the fraction of $\sigma_{total}$

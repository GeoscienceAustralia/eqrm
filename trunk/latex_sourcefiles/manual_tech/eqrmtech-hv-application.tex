...
\item \texttt{area}: Area of the source zone in km$^2$.  This is
  required but currently the value is not used.
...
\texttt{strike\_slip} or \texttt{unspecified}.
...
\item \texttt{scaling\_rule}: Defines the set of scaling rules which
  ...
  \texttt{Leonard\_SCR} (see \citet{eqrm_Wells94} and \citet{eqrm_Leonard10}).
...
\texttt{"Atkinson06\_soil"} $\Rightarrow$ Atkinson and Boore (2006) model for soil \\
...
\texttt{"Zhao\_2006\_intraslab"} $\Rightarrow$ Zhao \textit{et al.} (2006)
model for earthquakes \\
    \hspace{8em} on the subduction interface near Japan \\
\texttt{"Abrahamson\_Silva\_1997"} $\Rightarrow$ Abrahamson and Silva (1997)
eathquake model\\
\hline
\end{tabular}
...
\item \texttt{STRUCTURE\_CATEGORY}: Type of structure. Currently \texttt{BUILDING} only
\item \texttt{HAZUS\_USAGE}: HAZUS usage\index{building usgage!HAZUS}
  classification (\tref{tab:tab:grids-HAZUSusage})
...
\item \texttt{FCB\_USAGE}: FCB usage type (\tref{tab:grids-FCB})
...
classification\index{building usgage!HAZUS} \citep{dr_FEMA99b}.}
...
\hline
\\
Filename: \texttt{bridgedb\_<site\_tag><site\_db\_tag>.csv} \\
\\
\hline
\end{tabular}
\end{center}

The site file for bridges is a csv file containing a header and a list
of points at which the ground motion will be computed, and bridge damage 
estimated. An example is given below in the grey shaded box:
 
\lstinputlisting{xml_examples/bridgedb_newc.csv}

\textbf{Parameters in the hazard site file:}
\begin{itemize}
\item \texttt{Latitude}: Latitude of the bridge of interest.
\item \texttt{Longitude}: Longitude of the bridge of interest.
\item \texttt{STRUCTURE\_CLASSIFICATION}: HAZUS bridge type. See 
 \tref{tab:hazus-bridge-types} for a description of the types.
\item \texttt{STRUCTURE\_CATEGORY}: Type of structure. Currently \texttt{BRIDGE} 
 only.
\item \texttt{SKEW}: Bridge skew (degrees).
\item \texttt{SPAN}: Number of spans in the bridge.
\item \texttt{SITE\_CLASS}: Regolith site class. Typically, this is defined
by a letter. Note that the value of this parameter must match with
an amplification factor defined in the amplification file (see
\sref{sec:-appl-amplification}).
...
\begin{table}
 \caption{Definitions of the HAZUS Highway Bridge Classification.} 
 \vspace{0.8em} \label{tab:hazus-bridge-types}.
\centering
 {\footnotesize .
\begin{tabular}{|c|p{0.8\textwidth}|}
\hline
...
\hline
\\
Filename: \texttt{<site\_tag>\_popexp.csv} \\
\\
\hline
\end{tabular}
\end{center}

The site file for fatalities is a csv file containing a header and a list
of points at which the ground motion will be computed and converted to Modified 
Mercali Intensity (MMI), with which the population fatalities are estimated. 
An example is given below in the grey shaded box:
 
\lstinputlisting{xml_examples/java_popexp.csv}
...
\item \texttt{VS30}: Average velocity in the top 30\,m (i.e. $V_{s30}$). This is used
to incorporate regolith for GMPEs with a $V_{s30}$ term.
\item \texttt{POPULATION}: Population of the point of interest. An estimate of 
fatalities comes from the fatality rate as per the formula from USGS Open-File
Report 2009-1136 multiplied by \texttt{POPULATION}.
\end{itemize}
...
\section{Vulnerability File}
\label{sec:-appl-vulnerability}

\begin{center}
\begin{tabular}{|c|}
\hline
\\
Filename: \texttt{<site\_tag>\_vulnerability.xml} \\
\\
\hline
\end{tabular}
\end{center}

Vulnerability curves in EQRM use the Natural Hazard's Risk Markup Language 
(NRML) specified by The GEM Foundation for use in OpenQuake. An example 
vulnerability file is shown in the grey box below.

\lstinputlisting[language=XML]{xml_examples/padang_vulnerability.xml}

The top level attributes are defined in \texttt{discreteVulnerabilitySet}: \\
...
EQRM supports a single loss category option, \texttt{economic\_loss}, 
and the two other attributes are not currently used. \\
\\
The curves in the set are all based on the intensity measure levels defined in 
\texttt{IML}: \\
...
This is a list of intensity measure levels specified by the intensity measure 
type (IMT) attribute. EQRM supports conversion from spectral acceleration to
Modified Mercalli Intensity, \texttt{MMI}, as per the example above. \\
\\
The mean loss ratio and sigma are defined for each site type in 
\texttt{discreteVulnerability}: \\
...
Where:
\begin{itemize}
\item \texttt{vulnerabiltyFunctionID}: This specifies the site type that for 
this curve. It must match the \texttt{STRUCTURE\_CLASSIFICATION} defined in the 
\texttt{sitedb} file.
\item \texttt{probabilisticDistribution}: Options are \texttt{LN} (lognormal) 
and \texttt{N} (normal). The sampling technique will depend on the EQRM input 
parameter \texttt{vulnerability\_variability\_method}.
\item \texttt{lossRatio}: Mean loss values that correspond to the configured 
\texttt{IML}.
\item \texttt{coefficientsVariation}: Coefficient of variation that corresponds 
to the configured \texttt{lossRatio}, where 
\begin{equation}
CoV = \frac{\sigma}{\mu}
\end{equation}
\end{itemize}

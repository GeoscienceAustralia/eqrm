...
#FIXME - make a class 'polygon' <- 'complex_polygon' <- '_intersection_polygon'
...
class polygon_object(object):
    ...
    def contains_point(self,point, use_cach=True):
        ...
        if use_cach is True:
            try: answer =  self._precomputed_points[point]
            except KeyError:
    ...
    def difference(self,polygon2):
        ...
        if difference.area == 0: return empty_polygon
        if difference.area == self.area: return self
        return difference
    ...
    def union(self,polygon2):
        ...
        if union.area == self.area: return self
        return union
...
class polygon_difference(compound_polygon):
    ...
    def contains_point(self,point):
        ...
        """
        if not self.polygon1.contains_point(point): return False
        if self.polygon2.contains_point(point): return False
        return True
###############################################################################
class polygon_union(compound_polygon):
    ...
    def contains_point(self,point):
        ...
        Return True if polygon contains point, otherwise return False.
        """
        if self.polygon1.contains_point(point): return True
        if self.polygon2.contains_point(point): return True
        return False
###############################################################################

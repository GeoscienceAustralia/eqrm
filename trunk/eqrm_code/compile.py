...
def compile(FNs=None, CC=None, LD=None, SFLAG=None, verbose=1):
    ...
    if not isinstance(FNs, types.ListType):
        FNs = [FNs]
    ...
    if sys.platform == 'sunos5':  # Solaris
        ...
        if CC:
            ...
            # Some systems require this (a security measure?)
            compiler = 'gcc.exe'
        if LD:
            loader = LD
        else:
        ...
        if verbose:
            print "Unrecognised platform %s - revert to default"\
                % sys.platform
...
# NumPy        s = '%s -c %s -I"%s" -I"%s" -o "%s.o" -Wall -O3'\
    ...
    if libs is "":
        s = '%s -%s %s -o %s.%s -lm' % (loader,
                                        sharedflag, object_files, root1, libext)
    else:
        s = '%s -%s %s -o %s.%s "%s" -lm' % (loader,
                                             sharedflag, object_files, root1, libext, libs)
    if verbose:
        print s
    else:
...
def set_python_dll_path():
    ...
    # Hacky - fix if you want
        libs = os.path.join(
            os.environ["SYSTEMROOT"] + os.sep + 'system32',
            dllfilename)
    if access(libs, F_OK) == 0:
        # Hacky - fix if you want
        libs = os.path.join(

...
def main(parameter_handle,
    ...
    #print 'ENDING Creating synthetic event catalogue'
    #print 'STARTING Loading in sites'
    
    if THE_PARAM_T.run_type == 'risk': # Do builing damage
        
        building_par_file = build_par_file(THE_PARAM_T.buildpars_flag)
        #bridge_parameters=THE_PARAM_T.default_input_dir+bridgepars

        # Find location of site database (i.e. building database)
        # and get FID
        site_file='sitedb_'+THE_PARAM_T.site_tag+THE_PARAM_T.site_db_tag+'.csv'
        site_file=get_local_or_default(site_file,
                                       THE_PARAM_T.default_input_dir,
                                       THE_PARAM_T.input_dir)
        # load in all the site information as an object
        sites = Structures.from_csv(
            site_file,
            building_parameters_table=building_par_file,
            default_input_dir=THE_PARAM_T.default_input_dir,
            ...
            force_btype_flag=THE_PARAM_T.force_btype_flag,
        ...
        if THE_PARAM_T.buildings_set_damping_Be_to_5_percent is True:
            sites.building_parameters['damping_Be']=(
                0.05+0*sites.building_parameters['damping_Be'])
        ...
        name=get_local_or_default(name,
                                  THE_PARAM_T.default_input_dir,
                                  THE_PARAM_T.input_dir)        
        sites=Sites.from_csv(name,SITE_CLASS=str)
    else:
        raise ValueError("ran out of options for runtype")
        # do attenuation
    ...
    ground_motion_distribution = Log_normal_distribution(
        THE_PARAM_T.atten_use_variability,
    ...
    if THE_PARAM_T.use_amplification is True:
        regolith_amp_distribution = Log_normal_distribution(
            THE_PARAM_T.amp_use_variability,
        ...
        amp_factor_file=get_local_or_default(amp_factor_file,
                                             THE_PARAM_T.default_input_dir,
                                             THE_PARAM_T.input_dir)
        soil_amplification_model = Regolith_amplification_model.from_xml(
    ...
    msg = 'Number of atten_periods=' \
          + str(len(THE_PARAM_T.return_periods))
    log.debug(msg)
    ...
    if THE_PARAM_T.save_hazard_map is True:   
        bedrock_hazard=zeros((array_size,len(THE_PARAM_T.atten_periods),
                              len(THE_PARAM_T.return_periods)),dtype=float)
        if THE_PARAM_T.use_amplification is True:
    ...
    if THE_PARAM_T.save_motion is True:
        bedrock_SA_all=zeros((array_size,NUM_SITES_PER_SITE_LOOP,
                              len(THE_PARAM_T.atten_periods)),dtype=float)
        if THE_PARAM_T.use_amplification is True:
    ...
    if THE_PARAM_T.save_total_financial_loss is True:
        total_building_loss = zeros((array_size,
                                     num_psudo_events),
                                    dtype=float)
    if THE_PARAM_T.save_building_loss is True: 
        building_loss = zeros((array_size,
                               num_psudo_events),
                              dtype=float)
    if THE_PARAM_T.save_contents_loss is True: 
        contents_loss = zeros((array_size,
                               num_psudo_events),
                              dtype=float)
    ...
    for i in range(array_size):
        ...
        bedrock_SA_pdf=ground_motion_calc.distribution(
            event_set=event_set,
            sites=sites)
        ...
        # bedrock_SA dimensions (1, psudo-events, periods)
        ...
        distances=sites.distances_from_event_set( \
            pseudo_event_set).distance('Joyner_Boore')
        ...
        if THE_PARAM_T.use_amplification is True:
            soil_SA_pdf=soil_amplification_model.distribution(
                bedrock_SA,sites.attributes['SITE_CLASS'],
                pseudo_event_set.Mw,THE_PARAM_T.atten_periods)
        ...
        if soil_SA is not None:
            ...
            new_soil_SA, _, _ = do_collapse_logic_tree(
                ...
                pseudo_event_set.attenuation_weights, THE_PARAM_T)
        ...
        if THE_PARAM_T.save_hazard_map is True:
            for j in range(len(THE_PARAM_T.atten_periods)):
                bedrock_hazard[i,j]=hzd_do_value(
                    new_bedrock_SA[:,:,j], event_set.event_activity,
                    1.0/array(THE_PARAM_T.return_periods))
                if soil_SA is not None:
                    soil_hazard[i,j]=hzd_do_value(
                        new_soil_SA[:,:,j], event_set.event_activity,
                        1.0/array(THE_PARAM_T.return_periods))       
        ...
        if THE_PARAM_T.run_type == "risk":
            ...
            total_loss, damage = calc_total_loss(
                sites,SA,THE_PARAM_T,
                pseudo_event_set.Mw)
    ...
    if THE_PARAM_T.save_prob_structural_damage is True and \
           num_psudo_events == 1 and \
           THE_PARAM_T.run_type == 'risk' and parallel.lo != parallel.hi:
        # if parallel.lo != parallel.hi
        # No sites were investigated.
        file = save_damage(THE_PARAM_T.output_dir,
    ...
    if (THE_PARAM_T.save_motion is True or \
        ...
        files = save_distances(THE_PARAM_T,
                              sites=all_sites,
                              event_set=event_set,
                              compress=THE_PARAM_T.compress_output,
                              parallel_tag=parallel.file_tag)
    ...
    if (THE_PARAM_T.save_total_financial_loss is True or \
        ...
        THE_PARAM_T.save_contents_loss is True) and \
        parallel.lo != parallel.hi:
        file = save_structures(THE_PARAM_T,
                               all_sites,
                               compress=THE_PARAM_T.compress_output,
                               parallel_tag=parallel.file_tag,
                               write_title=(parallel.rank == False))
    ...
    if THE_PARAM_T.save_building_loss is True and parallel.lo != parallel.hi:
        new_building_loss, _, _ = do_collapse_logic_tree( \
            building_loss[:,:,newaxis],
            pseudo_event_set.index,
            pseudo_event_set.attenuation_weights,
            THE_PARAM_T)
         # collapse out fake periods axis
...
#         row_files_that_parallel_splits.append(file)
    ...
    if THE_PARAM_T.save_contents_loss is True and parallel.lo != parallel.hi:
        new_contents_loss, _, _ = do_collapse_logic_tree( \
            contents_loss[:,:,newaxis],
            pseudo_event_set.index,
            pseudo_event_set.attenuation_weights,
            THE_PARAM_T)
...
#         row_files_that_parallel_splits.append(file)
    ...
    if parallel.is_parallel is True and parallel.rank == 0:
    ...
    #parallel.barrier()
    #print "Rank %i on %s" % (parallel.rank,parallel.node)

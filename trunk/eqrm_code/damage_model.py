"""
 Title: damage model
  ...
  ModifiedDate: $Date: 2010-03-26 16:12:23 +1100 (Fri, 26 Mar 2010) $
...
class Damage_model(object):
    ...
    def __init__(self, structures, SA, periods, magnitudes,
                 csm_use_variability, csm_standard_deviation, # THE_PARAM_T values
                 csm_params=None):
        """
        ...
        structures - a Structures instance - But what is actually
          needed from it?
  ...
  #warning: this option will cause divide by zero warnings in make_fragility.m
            ...
            raise 'ERORR in prep_build_vun: csm_use_variability not properly defined'
        
        SA,SD=self.get_building_displacement()
        SA=SA.round(4)
        SD=SD.round(4)
        #print 'get_building_states SD(mm) SA(g)',SD,SA
        #print 'get_building_states SD(m) = '+ str(SD[0][0]/1000) + ' SA(m/s2) = ' + str(SA[0][0]*9.80665)
    ...
    def aggregated_building_loss(self,ci=None,loss_aus_contents=0):
        loss_ratio,total_loss=self.building_loss(
            ci=ci,loss_aus_contents=loss_aus_contents)
...
def state_probability(threshold,beta,value):
    ...
    #print "state_probability p before", p
    reduce_cumulative_to_pdf(p)
    #print "state_probability p after", p
    return p
...
def calc_total_loss(sites, SA, THE_PARAM_T, event_set_Mw):
    ...
    damage_model=Damage_model(sites,SA,THE_PARAM_T.atten_periods,
        ...
        'rtol':THE_PARAM_T.csm_SDcr_tolerance_percentage/100.0,
        'csm_damping_max_iterations':THE_PARAM_T.csm_damping_max_iterations,
        'sdtcap':THE_PARAM_T.csm_standard_deviation, #FIXME sdt -> std
        'csm_use_variability':THE_PARAM_T.csm_use_variability,
        'csm_variability_method':THE_PARAM_T.csm_variability_method,
        'csm_hysteretic_damping':THE_PARAM_T.csm_hysteretic_damping,
        'atten_rescale_curve_from_pga':THE_PARAM_T.atten_rescale_curve_from_pga,
        'atten_cutoff_max_spectral_displacement':THE_PARAM_T.atten_cutoff_max_spectral_displacement,
